...
#	define   LOCK_MEMORY_(memory, size)   shim_lock_memory( memory, size )
#	define UNLOCK_MEMORY_(memory, size) shim_unlock_memory( memory, size )
#else
#	define   LOCK_MEMORY_(memory, size) /*Null macro.*/
#	define UNLOCK_MEMORY_(memory, size) /*Null macro.*/
#endif
...
Password_Generator::Password_Generator (C_Argument_Map &c_arg_map)
{
	...
	static_assert (Number_Random_Bytes % sizeof(uint64_t) == 0,
		       "The number of random bytes must be divisible into words.");
	static constexpr int Number_Random_Words = Number_Random_Bytes / sizeof(uint64_t);
	struct {
		typename CSPRNG_f::Data csprng_data;
		uint64_t                random_bytes  [Number_Random_Words];
		uint8_t                 entropy_bytes [Entropy_Buffer_Bytes];
	...
	CSPRNG_f::get( &crypto.csprng_data,
		       reinterpret_cast<uint8_t*>(crypto.random_bytes),
		       Number_Random_Bytes );
	}
	...
	if (use_formatting) {
		...
		uint8_t *pwd = crypto.password;
		while (chars_left >= Chars_Per_Block) {
			std::fwrite( pwd, sizeof(uint8_t), Chars_Per_Block, stdout );
			pwd        += Chars_Per_Block;
		...
		if (chars_left > 0) {
			std::fwrite( pwd, sizeof(uint8_t), chars_left, stdout );
		}
		if (blocks_left != Blocks_Per_Line)
			std::putchar( '\n' );
	} else {
		std::fwrite( crypto.password, sizeof(uint8_t), size, stdout );
		std::putchar( '\n' );
	}
...
Password_Generator::process_arguments_ (C_Argument_Map &c_arg_map)
{
	...
	if( count == 0 )
		SHIM_ERRX ("Error: Called with no arguments.\n");
...
Password_Generator::set_character_table_ ()
{
	...
	uint8_t *character= character_table;
...
int
Password_Generator::generate_password_ (uint8_t *        SHIM_RESTRICT password,
					uint64_t const * SHIM_RESTRICT random_words)
{
	...
	uint64_t const local_limit = Upper_Limit + (Upper_Limit % static_cast<uint64_t>(number_characters));
	uint64_t const quanta_per_character = local_limit / static_cast<uint64_t>(number_characters);

	std::memset( password, 0, Password_Buffer_Bytes );
	uint64_t offset;
	for (int i = 0; i < requested_password_size; ++i) {
		uint64_t p = random_words[ i ];
		if (p <= local_limit) {
			uint64_t const p_prime = p - (p % quanta_per_character);
...
#endif
void
Password_Generator::supplement_entropy_ (typename CSPRNG_f::Data * SHIM_RESTRICT csprng_data,
		                         uint8_t                 * SHIM_RESTRICT buffer)
{
	...
	static constexpr auto &Entropy_Prompt = "Please input up to 120 random characters." PROMPT_ ;
	uint8_t *hash = buffer;
	uint8_t *keyboard_input = hash + Hash_Size;
	using Terminal_UI_f = ssc::Terminal_UI_F<uint8_t, Entropy_Buffer_Bytes>;
	Terminal_UI_f::init();
	int num_input_chars = Terminal_UI_f::obtain_password( keyboard_input, Entropy_Prompt );
	Terminal_UI_f::end();
	Skein_f::hash_native( &csprng_data->skein_data,
			      hash,
			      keyboard_input,
			      num_input_chars );
...
void
Password_Generator::process_pw_size_ (char const * SHIM_RESTRICT number, int const size)
{
	if( size > 3 )
		SHIM_ERRX ("Error: Maximum password size is 125 characters.\n");
	else if( size < 1 )
		SHIM_ERRX ("Error: Minimum password size is 1 character.\n");
	if( temp_cstr != nullptr )
		SHIM_ERRX ("Error: temp_cstr not already nullptr.\n");
		...
		SHIM_ERRX (Generic_Error::Alloc_Failure);
	{
		...
		if( num_digits < 1 || num_digits > 3)
			SHIM_ERRX ("Error: Minimum password size is 1 character; maximum password size is 125 characters.\n");
